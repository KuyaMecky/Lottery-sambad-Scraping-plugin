<?php
/**
 * Plugin Name: Lottery Scraper (Nagaland1pm Todays Result)
 * Description: A simple plugin to scrape a specific div from Lottery Sambad's Nagaland1pm Result result page.
 * Version: 1.2
...
function lottery_scraper_Nagaland1pm_Result_shortcode() {
    // Update the URL to the new page
    $url = 'https://lotterysambad.one/nagaland-state-lottery-result-1-pm/';
    ...
    if ($div) {
        ...
        $elements_to_remove = [
            ...
            '//a',
            '//div[@class="simplesocialbuttons simplesocial-simple-icons simplesocialbuttons_inline simplesocialbuttons-align-centered post-1065 page  simplesocialbuttons-inline-no-animation"]',
        ...
        foreach ($images as $img) {
            ...
            if (strpos($src, 'http') !== 0) {
                $img->setAttribute('src', 'https://lotterysambad.one/nagaland-state-lottery-result-1-pm/' . ltrim($src, '/'));
            }
        }
...
add_shortcode('Lottery_Result_Nagaland1pm', 'lottery_scraper_Nagaland1pm_Result_shortcode');
